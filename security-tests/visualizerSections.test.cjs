...
//
// Tests the sections data structure in src/lib/visualizerSections.js.

const { describe, test } = require("node:test");
const assert = require("node:assert/strict");

// Inline the sections export from src/lib/visualizerSections.js.
const sections = [
  {
    title: "Code Lab",
    slug: "code-lab",
...
describe("sections structure", () => {
  ...
  test("each section has required fields: title, slug, desc, subsections", () => {
    for (const section of sections) {
      assert.ok(typeof section.title === "string" && section.title.length > 0,
        `section title must be non-empty string, got: ${JSON.stringify(section.title)}`);
      assert.ok(typeof section.slug === "string" && section.slug.length > 0,
        `section slug must be non-empty string, got: ${JSON.stringify(section.slug)}`);
      assert.ok(typeof section.desc === "string" && section.desc.length > 0,
        `section desc must be non-empty string, got: ${JSON.stringify(section.desc)}`);
      assert.ok(Array.isArray(section.subsections) && section.subsections.length > 0,
        `section.subsections must be non-empty array, got: ${JSON.stringify(section.subsections)}`);
    }
  });
  ...
  test("all section slugs are unique", () => {
    ...
    assert.strictEqual(slugs.length, uniqueSlugs.length,
      `duplicate slugs found: ${slugs.filter((s, i) => slugs.indexOf(s) !== i).join(", ")}`);
  });

  test("all section slugs are kebab-case (lowercase, letters, numbers, hyphens only)", () => {
    const kebabRe = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
    for (const section of sections) {
      assert.ok(kebabRe.test(section.slug),
        `slug "${section.slug}" is not kebab-case`);
    }
  });

  test("each subsection has title and items array", () => {
    for (const section of sections) {
      for (const subsection of section.subsections) {
        assert.ok(typeof subsection.title === "string" && subsection.title.length > 0,
          `subsection title must be non-empty string in section ${section.slug}`);
        assert.ok(Array.isArray(subsection.items) && subsection.items.length > 0,
          `subsection items must be non-empty array in section ${section.slug}`);
      }
    }
  });
      ...
      for (const subsection of section.subsections) {
        for (const item of subsection.items) {
          assert.ok(typeof item.name === "string" && item.name.length > 0,
            `item name must be non-empty string in section ${section.slug}`);
          assert.ok(typeof item.path === "string" && item.path.length > 0,
            `item path must be non-empty string in section ${section.slug}`);
        }
      }
    }
  });

  test("all paths start with /visualizer/", () => {
    for (const section of sections) {
      for (const subsection of section.subsections) {
        for (const item of subsection.items) {
          assert.ok(item.path.startsWith("/visualizer/"),
            `path "${item.path}" in section ${section.slug} must start with /visualizer/`);
        }
      }
    }
  });

  test("sections contains expected top-level slugs", () => {
    const slugs = sections.map((s) => s.slug);
    const expected = ["array", "recursion", "stack", "queue", "linked-list", "tree", "hashmap", "graph", "dp", "ai"];
    for (const exp of expected) {
      assert.ok(slugs.includes(exp), `expected slug "${exp}" not found in sections`);
    }
  });
});
