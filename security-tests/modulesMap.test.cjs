...
//
// Tests the MODULE_MAPS constant export in src/lib/modulesMap.js.

const { test, describe } = require('node:test');
const assert = require('node:assert/strict');

// ── Inlined source from src/lib/modulesMap.js ───────────────────────────
...
const KNOWN_ALGORITHM_KEYS = [
  'linearSearch', 'binarySearch', 'bubbleSort', 'selectionSort',
  'insertionSort', 'mergeSort', 'quickSort',
  'stackArray', 'queueArray', 'trie', 'heapSort',
  'hashmapInsert', 'hashmapSearch', 'hashmapDelete',
  'recursionFibonacci', 'recursionFactorial', 'recursionHanoi',
];

const UUID_RE = /^[a-z0-9-]+$/;
...
describe('MODULE_MAPS', () => {
  ...
  test('is non-empty', () => {
    ...
    assert.ok(keys.length > 0, 'MODULE_MAPS should not be empty');
    assert.ok(keys.length >= 50, `MODULE_MAPS should have at least 50 keys, found ${keys.length}`);
  });

  test('known algorithm keys all exist', () => {
    for (const key of KNOWN_ALGORITHM_KEYS) {
      assert.ok(
        key in MODULE_MAPS,
        `Expected key '${key}' to exist in MODULE_MAPS`,
      );
    }
  });
  ...
  test('all keys are non-empty strings', () => {
    for (const key of Object.keys(MODULE_MAPS)) {
      assert.strictEqual(
        typeof key,
        'string',
        `Key should be a string, got ${typeof key}`,
      );
      assert.ok(key.length > 0, 'Key should not be empty');
    }
  });

  test('known algorithm keys return non-empty UUID-like values', () => {
    for (const key of KNOWN_ALGORITHM_KEYS) {
      const value = MODULE_MAPS[key];
      assert.ok(
        value.length > 0,
        `Value for '${key}' should be non-empty`,
      );
    }
  });
  ...
  test('no duplicate values', () => {
    ...
    assert.strictEqual(
      values.length,
      unique.size,
      `MODULE_MAPS has duplicate values: ${values.filter((v, i) => values.indexOf(v) !== i).join(', ')}`,
    );
  });

  test('linearSearch maps to a valid-looking UUID', () => {
    const val = MODULE_MAPS.linearSearch;
    assert.ok(UUID_RE.test(val), `linearSearch value '${val}' does not look like a UUID`);
  });

  test('heapSort maps to a valid-looking UUID', () => {
    const val = MODULE_MAPS.heapSort;
    assert.ok(UUID_RE.test(val), `heapSort value '${val}' does not look like a UUID`);
  });
});
