...
export function useProblemBookmarks() {
  ...
  useEffect(() => {
    const loadBookmarks = async () => {
      ...
      let localBookmarks: ProblemBookmark[] = [];
      try {
        const stored = localStorage.getItem(STORAGE_KEY);
        if (stored) {
          localBookmarks = JSON.parse(stored) as ProblemBookmark[];
      ...
      if (user) {
        try {
          ...
          if (error) {
            ...
            const dbBookmarks: ProblemBookmark[] = data.map((row) => ({
              id: row.problem_id,
  ...
  const toggleBookmark = async (problemId: string, topicSlug: string) => {
    ...
    if (isBookmarkedCurrently) {
      ...
      // Add bookmark
      const newBookmark: ProblemBookmark = {
        id: problemId,
        topicSlug,
