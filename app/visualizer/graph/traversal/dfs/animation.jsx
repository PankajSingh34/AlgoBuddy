...
const DfsAnimation = () => {
  ...
  const handleGo = (e) => {
    ...
    const order = [];
    setStack([...stk]);
    setMessage(`Starting DFS from node ${startNode}. Pushing ${startNode} onto stack.`);

    animateNode(startNode, "#EAB308");
    animateDFS(stk, vis, order);
  };

  const animateDFS = (stk, vis, order) => {
    const step = () => {
      if (stk.length === 0) {
        setIsAnimating(false);
        setCurrentNode(null);
        setMessage(`DFS complete! Traversal order: [${order.join(" → ")}]`);
        return;
      }
      ...
      setCurrentNode(u);
      setMessage(`Popped node ${u}. Marking visited. Traversal: [${order.join(" → ")}]`);

      animateNode(u, "#22C55E");
      gsap.to(nodeRefs.current[u], { scale: 1.2, duration: 0.3, yoyo: true, repeat: 1 });

      const neighbors = [...adjList[u]].reverse();
      let delay = 500;

      neighbors.forEach((v) => {
        if (!vis.has(v) && !stk.includes(v)) {
          stk.push(v);
          const pushMessage = `Pushed ${v} onto stack. Neighbors of ${u}`;
      ...
      animationRef.current = setTimeout(step, delay + 300);
    };
  ...
  return (
    <main className="container mx-auto px-6 pb-4">
      <p className="text-lg text-center text-gray-600 dark:text-gray-400 mb-8">
        Watch DFS traverse the graph depth-first. The stack grows as it goes deeper and shrinks during backtracking.
      </p>
