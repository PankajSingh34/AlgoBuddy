...
const DfsQuiz = () => {
  const questions = [
    {
      question: "What data structure does iterative DFS use to manage vertices to visit?",
      options: [
        "Queue",
        "Stack",
        "Priority Queue",
      ...
      correctAnswer: 1,
      explanation: "DFS uses a stack (either the call stack for recursion or an explicit stack for iteration) to implement backtracking."
    },
    {
      question: "What is the time complexity of DFS when using an adjacency list?",
      options: [
        "O(V)",
        "O(E)",
        "O(V + E)",
      ...
      correctAnswer: 2,
      explanation: "DFS visits each vertex once (O(V)) and processes each edge once (O(E)), giving O(V + E)."
    },
    {
      question: "Which of these is NOT a common application of DFS?",
      options: [
        "Topological sorting",
        "Cycle detection",
      ...
      correctAnswer: 2,
      explanation: "BFS (not DFS) finds the shortest path in unweighted graphs. DFS is used for topological sort, cycle detection, and connected components."
    },
    {
      question: "How does recursive DFS track visited vertices?",
      options: [
        "Using a queue",
        "Using a visited set passed between recursive calls",
        "Using a priority queue",
        "Using timestamps only"
      ],
      correctAnswer: 1,
      explanation: "A visited set (or boolean array) is passed through recursive calls to ensure vertices aren't revisited."
    },
    {
      question: "In DFS, what order are vertices processed relative to their neighbors?",
      options: [
        "All neighbors first, then deeper",
        "Go as deep as possible, then backtrack",
        "Random order",
        "Level by level"
      ],
      correctAnswer: 1,
      explanation: "DFS explores as deep as possible along each branch before backtracking to try alternative paths."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "In the worst case (a linear chain), the recursion stack holds all V vertices, giving O(V) space."
    },
    {
      question: "What colors are commonly used to track DFS vertex states?",
      options: [
        "Red, green, blue",
      ...
      correctAnswer: 1,
      explanation: "White = unvisited, gray = currently in the recursion stack, black = all neighbors processed."
    },
    {
      question: "How does DFS detect cycles in a directed graph?",
      options: [
        "Counting edges",
        "Checking if a gray vertex is encountered during traversal",
        "Using a queue",
        "Counting all vertices"
      ],
      correctAnswer: 1,
      explanation: "If DFS encounters a vertex that is currently gray (in the current recursion stack), a back edge exists, indicating a cycle."
    },
    {
      question: "In the graph 0-1-2-3-4 (line), starting DFS from 0, what is a valid traversal order?",
      options: [
        "0, 1, 2, 3, 4",
        "4, 3, 2, 1, 0",
        "0, 2, 1, 3, 4",
      ...
      correctAnswer: 0,
      explanation: "DFS goes as deep as possible: 0 → 1 → 2 → 3 → 4, then backtracks all the way."
    },
    {
      question: "What is a key difference between DFS and BFS?",
      options: [
        "DFS uses more memory than BFS",
        "DFS does not guarantee shortest paths, BFS does",
        "DFS can only be used on trees",
        "DFS is always faster than BFS"
      ],
      correctAnswer: 1,
      explanation: "BFS finds the shortest path in unweighted graphs. DFS explores depth-first and may find a longer path first."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    const weak = [];
    if (answers[0] !== questions[0].correctAnswer) weak.push("DFS stack data structure");
    if (answers[1] !== questions[1].correctAnswer) weak.push("time complexity");
    if (answers[2] !== questions[2].correctAnswer) weak.push("DFS applications");
    if (answers[3] !== questions[3].correctAnswer) weak.push("visited set in recursion");
    if (answers[4] !== questions[4].correctAnswer) weak.push("DFS traversal order");
    if (answers[5] !== questions[5].correctAnswer) weak.push("space complexity");
    if (answers[6] !== questions[6].correctAnswer) weak.push("vertex coloring states");
    if (answers[7] !== questions[7].correctAnswer) weak.push("cycle detection");
    if (answers[8] !== questions[8].correctAnswer) weak.push("DFS on a linear graph");
    if (answers[9] !== questions[9].correctAnswer) weak.push("DFS vs BFS comparison");
    return weak.length > 0 ? `Focus on improving: ${weak.join(', ')}.` : "Perfect! You've mastered all DFS concepts!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">DFS Quiz Challenge</h2>
