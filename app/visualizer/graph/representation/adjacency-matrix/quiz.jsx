...
const AdjacencyMatrixQuiz = () => {
  const questions = [
    {
      ...
      correctAnswer: 2,
      explanation: "An adjacency matrix always stores V² entries regardless of how many edges exist, so space complexity is O(V²)."
    },
    {
      question: "How long does it take to check if an edge exists between two vertices in an adjacency matrix?",
      options: [
        "O(1)",
        "O(V)",
        "O(log V)",
      ...
      correctAnswer: 0,
      explanation: "Edge lookup in an adjacency matrix is O(1) because we can directly index matrix[u][v]."
    },
    {
      question: "When is an adjacency matrix the most appropriate graph representation?",
      options: [
        "When the graph is sparse",
        "When the graph is dense (close to V² edges)",
        "When the graph has no edges",
        "When vertices have high degree"
      ],
      correctAnswer: 1,
      explanation: "Adjacency matrices are best for dense graphs where the O(V²) space is justified by the large number of edges."
    },
    {
      question: "In an undirected graph's adjacency matrix, what property does the matrix have?",
      options: [
      ...
      correctAnswer: 0,
      explanation: "For undirected graphs, matrix[i][j] = matrix[j][i], making the matrix symmetric along the diagonal."
    },
    {
      question: "How long does it take to find all neighbors of a vertex using an adjacency matrix?",
      options: [
        "O(1)",
        "O(V)",
        "O(degree(v))",
      ...
      correctAnswer: 1,
      explanation: "To find all neighbors of vertex u, you must scan the entire row u (all V entries), which takes O(V) time."
    },
    {
      question: "What does matrix[i][i] represent in an adjacency matrix?",
      options: [
        "The degree of vertex i",
        "A self-loop from vertex i to itself",
        "The weight of vertex i",
        "The distance from vertex i to itself"
      ],
      correctAnswer: 1,
      explanation: "The diagonal entry matrix[i][i] represents a self-loop (edge from vertex i to itself)."
    },
    {
      question: "If a graph has 5 vertices and 4 edges, how many entries are stored in its adjacency matrix?",
      options: [
        "4",
        "5",
        "25",
      ...
      correctAnswer: 2,
      explanation: "A 5-vertex graph has a 5×5 matrix, storing 25 entries regardless of the number of edges."
    },
    {
      question: "What is the time complexity of adding an edge in an adjacency matrix?",
      options: [
        "O(1)",
        "O(V)",
        "O(E)",
      ...
      correctAnswer: 0,
      explanation: "Adding an edge is simply setting matrix[u][v] = 1, which is an O(1) operation."
    },
    {
      question: "In a weighted graph's adjacency matrix, what is stored instead of a 1?",
      options: [
        "Boolean true",
        "The edge weight",
      ...
      correctAnswer: 1,
      explanation: "For weighted graphs, matrix[i][j] stores the weight of the edge instead of just 1."
    },
    {
      question: "Which traversal over a row of an adjacency matrix takes O(V) time?",
      options: [
        "Adding an edge",
        "Removing an edge",
      ...
      correctAnswer: 2,
      explanation: "Finding all neighbors requires iterating over all V columns in the row to find non-zero entries, which is O(V)."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("space complexity of adjacency matrix");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("edge lookup time complexity");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("when to use adjacency matrix");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("undirected graph symmetry");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("finding neighbors");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("self-loops in matrix");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("matrix sizing");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("adding edges");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("weighted graphs");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("row traversal");
    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all adjacency matrix concepts!";
  };
      ...
      {showIntro ? (
        ...
        >
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">
            Adjacency Matrix Quiz Challenge
          </h2>
...
export default AdjacencyMatrixQuiz;
