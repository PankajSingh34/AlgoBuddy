...
const TopologicalSortQuiz = () => {
  const questions = [
    {
      ...
      correctAnswer: 1,
      explanation: "Topological sort is only defined for Directed Acyclic Graphs (DAGs). A cycle makes topological ordering impossible."
    },
    {
      question: "What happens if you try to topologically sort a graph with a cycle?",
      options: [
        "It still produces a valid ordering",
        "It throws an error",
        "Not all vertices will be in the order",
        "It ignores the cycle"
      ],
      correctAnswer: 2,
      explanation: "A cycle means there's no linear ordering satisfying all constraints. Some vertices will remain unprocessed, so the order will be incomplete."
    },
    {
      ...
      correctAnswer: 2,
      explanation: "Kahn's algorithm processes each vertex and each edge exactly once, giving O(V + E) time complexity."
    },
    {
      question: "In Kahn's algorithm, what does a vertex with in-degree 0 mean?",
      options: [
        "It has no outgoing edges",
        "It has no incoming edges (no dependencies)",
        "It has been processed",
        "It is part of a cycle"
      ],
      correctAnswer: 1,
      explanation: "An in-degree of 0 means the vertex has no incoming edges, so it has no prerequisites and can be placed next in the topological order."
    },
    {
      question: "Which data structure is used in Kahn's algorithm?",
      options: [
        "Stack",
        "Queue (or similar collection)",
        "Priority queue",
        "Hash table"
      ],
      correctAnswer: 1,
      explanation: "Kahn's algorithm uses a queue (or any collection) to store vertices with in-degree 0 that are ready to be processed."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "In-degree is the count of incoming edges. In topological sorting context, it represents the number of dependencies (prerequisites) a vertex has."
    },
    {
      question: "Which of the following is a real-world application of topological sort?",
      options: [
        "Sorting numbers in ascending order",
        "Build systems (like Make) resolving compilation dependencies",
        "Finding the shortest path between two cities",
        "Detecting communities in social networks"
      ],
      correctAnswer: 1,
      explanation: "Build systems use topological sort to determine the order of compilation based on file dependencies. Package managers also use it for dependency resolution."
    },
    {
      question: "What information does the DFS-based topological sort use?",
      options: [
        "In-degree counts",
        "Post-order (finish time) traversal",
        "Distance from source",
        "Edge weights"
      ],
      correctAnswer: 1,
      explanation: "DFS-based topological sort performs a DFS traversal and adds vertices to the order after all their descendants are processed (post-order), then reverses the result."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Kahn's algorithm requires O(V) extra space for the queue, in-degree array, and the result order list."
    },
    {
      question: "In Kahn's algorithm, when a vertex is processed, what happens to its outgoing edges?",
      options: [
        "They are removed from the graph",
        "The in-degree of each neighbor is decremented",
        "The edge weight is updated",
        "The neighbors are immediately processed"
      ],
      correctAnswer: 1,
      explanation: "When a vertex is processed, the in-degree of each of its neighbors is decremented by 1 (representing one fewer dependency)."
    },
  ];
  ...
  const resetQuiz = () => {
    setCurrentQuestion(0); setSelectedAnswer(null); setScore(0);
    setShowResult(false); setQuizCompleted(false);
    setAnswers(Array(questions.length).fill(null)); setShowIntro(true);
  };

  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("graph type requirements (DAG)");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("cycle detection behavior");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("time complexity");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("in-degree meaning");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("data structure used");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("in-degree definition");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("real-world applications");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("DFS-based approach");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("space complexity");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("edge processing logic");

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all Topological Sort concepts!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Topological Sort Quiz Challenge</h2>
              ...
              {questions[currentQuestion].options.map((option, index) => (
                <motion.div key={index} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  ...
                  <div className="flex items-center">
                    <span className={`font-medium mr-3 w-6 h-6 flex items-center justify-center rounded-full ${selectedAnswer === index ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-600'}`}>
...
export default TopologicalSortQuiz;
