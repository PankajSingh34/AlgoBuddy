...
const DijkstraQuiz = () => {
  const questions = [
    {
      ...
      options: [
        "Any weighted graph",
        "Weighted graph with non-negative edges only",
        "Unweighted graph only",
        "Directed graph only"
      ],
      correctAnswer: 1,
      explanation: "Dijkstra's algorithm requires all edge weights to be non-negative. With negative weights, the greedy approach fails and Bellman-Ford should be used."
    },
    {
      question: "What data structure is primarily used in Dijkstra's algorithm?",
      options: [
        "Stack",
        "Queue",
      ...
      correctAnswer: 2,
      explanation: "A min-priority queue (typically implemented with a binary heap) is used to efficiently extract the vertex with the smallest distance."
    },
    {
      ...
      correctAnswer: 2,
      explanation: "With a binary heap, Dijkstra runs in O((V + E) log V) — each of the V extract-min and E decrease-key operations takes O(log V)."
    },
    {
      question: "What operation is performed when a shorter path to a vertex is found?",
      options: [
        "Edge relaxation",
        "Edge contraction",
        "Graph pruning",
      ...
      correctAnswer: 0,
      explanation: "Edge relaxation updates the distance estimate of a neighboring vertex if a shorter path through the current vertex is discovered."
    },
    {
      question: "What happens when Dijkstra's algorithm processes a vertex with negative edge weights?",
      options: [
      ...
      correctAnswer: 1,
      explanation: "Negative edge weights break Dijkstra's greedy property — a vertex settled early might later find a shorter path via a negative edge, but it won't be reconsidered."
    },
    {
      question: "In Dijkstra's algorithm, what does it mean for a vertex to be 'settled'?",
      options: [
        "It has been visited but not fully processed",
        "Its shortest distance from source is finalized",
        "It is still in the priority queue",
        "It has no outgoing edges"
      ],
      correctAnswer: 1,
      explanation: "A settled vertex has its shortest distance finalized and will not be processed again. This is correct only with non-negative edge weights."
    },
    {
      question: "What will Dijkstra's algorithm return for a vertex unreachable from the source?",
      options: [
        "Zero",
        "Negative one",
        "Infinity",
      ...
      correctAnswer: 2,
      explanation: "Unreachable vertices retain their initial distance of infinity (or a sentinel value) since no path from the source exists."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Dijkstra's algorithm powers GPS navigation systems, mapping apps, and network routing protocols like OSPF and IS-IS."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Dijkstra requires O(V) space for storing distances, visited status, and the priority queue."
    },
    {
      question: "How many times is each vertex extracted from the priority queue in Dijkstra's algorithm?",
      options: [
        "Exactly once",
        "At most once",
        "At most E times",
      ...
      correctAnswer: 0,
      explanation: "Each vertex is extracted exactly once, when its shortest distance is finalized (settled). After extraction, it won't be re-added."
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("graph requirements for Dijkstra");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("data structures used by Dijkstra");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("time complexity analysis");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("edge relaxation concept");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("negative edge weight handling");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("vertex settlement meaning");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("unreachable vertex behavior");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("real-world applications");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("space complexity");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("vertex extraction count");

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all Dijkstra's algorithm concepts!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Dijkstra's Algorithm Quiz Challenge</h2>
