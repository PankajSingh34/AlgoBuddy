...
const PrimQuiz = () => {
  const questions = [
    {
      ...
      correctAnswer: 1,
      explanation: "Prim's algorithm finds a Minimum Spanning Tree — a subset of edges that connects all vertices with minimum total weight."
    },
    {
      question: "What type of algorithmic approach does Prim's use?",
      options: [
        "Divide and Conquer",
        "Dynamic Programming",
        "Greedy",
      ...
      correctAnswer: 2,
      explanation: "Prim's is a greedy algorithm — it makes the locally optimal choice (smallest edge crossing the cut) at each step."
    },
    {
      question: "Which property guarantees the correctness of Prim's algorithm?",
      options: [
        "Triangle inequality",
        "Cut property",
      ...
      correctAnswer: 1,
      explanation: "The cut property ensures that the minimum weight edge crossing any cut belongs to some MST, guaranteeing the greedy choice is safe."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "With a binary heap, Prim's runs in O((V+E) log V) — each extract-min and decrease-key operation takes O(log V)."
    },
    {
      question: "How does Prim's algorithm differ from Kruskal's?",
      options: [
        "Prim's works on directed graphs only",
        "Prim's grows a single tree; Kruskal's grows a forest",
        "Prim's sorts edges first; Kruskal's doesn't",
        "They are identical"
      ],
      correctAnswer: 1,
      explanation: "Prim's grows a single tree from one vertex, while Kruskal's initially treats each vertex as a tree and merges them."
    },
    {
      question: "What kind of graph does Prim's algorithm work on?",
      options: [
        "Directed graph only",
      ...
      correctAnswer: 1,
      explanation: "Prim's algorithm works on undirected graphs. The MST concept is defined for undirected graphs."
    },
    {
      question: "In Prim's algorithm, what is the 'cut'?",
      options: [
        "A separation of the graph into visited and unvisited vertices",
        "A removal of the heaviest edge",
        "A division of the graph into two equal halves",
        "A set of edges to be deleted"
      ],
      correctAnswer: 0,
      explanation: "The cut separates vertices already in the MST from those not yet in it. The algorithm always picks the cheapest edge crossing this cut."
    },
    {
      question: "What is the total number of edges in an MST with V vertices?",
      options: [
        "V",
        "V - 1",
        "V + 1",
      ...
      correctAnswer: 1,
      explanation: "Any spanning tree on V vertices has exactly V - 1 edges, which is the minimum needed to connect all vertices."
    },
    {
      question: "Which of the following is a real-world application of Prim's algorithm?",
      options: [
        "Sorting a database",
      ...
      correctAnswer: 1,
      explanation: "Prim's is used in network design — laying fiber optic cables, building roads, and designing circuit boards with minimal cost."
    },
    {
      question: "What happens if the input graph is disconnected for Prim's algorithm?",
      options: [
        "It finds the MST of the connected component containing the start vertex",
        "It throws an error",
        "It connects all components",
        "It finds a spanning forest"
      ],
      correctAnswer: 0,
      explanation: "Prim's algorithm only processes vertices reachable from the start vertex. It will find the MST of that component, not the full graph."
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("what Prim's algorithm finds");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("algorithmic approach (greedy)");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("cut property correctness");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("time complexity");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("difference from Kruskal's");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("graph type requirements");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("cut definition in Prim's");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("MST edge count");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("real-world applications");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("disconnected graph behavior");

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all Prim's algorithm concepts!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Prim's Algorithm Quiz Challenge</h2>
