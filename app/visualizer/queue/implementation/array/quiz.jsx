...
const ArrayQueueQuiz = () => {
  const questions = [
    {
      question: "What pointers are used in a circular array queue?",
      options: ["head and tail", "front and rear", "top and bottom", "left and right"],
      correctAnswer: 1,
      explanation: "A circular array queue uses front (pointing to the first element) and rear (pointing to the last element) pointers.",
    },
    {
      question: "What problem does the circular queue solve over the linear queue?",
      options: ["Speed of enqueue", "Memory reusability (wrap-around)", "Security of data", "Sorting elements"],
      correctAnswer: 1,
      explanation: "A circular queue reuses empty spaces at the beginning of the array after dequeuing, by wrapping around to index 0.",
    },
    {
      question: "What is the time complexity of enqueue and dequeue in a circular array queue?",
      options: ["O(n)", "O(log n)", "O(1)", "Depends on size"],
      correctAnswer: 2,
      explanation: "Both enqueue and dequeue operations are O(1) as they only modify front/rear pointers and access fixed indices.",
    },
    {
      question: "In a circular queue with MAX_SIZE=5, if rear=4 and front=2, where does the next enqueue place the element?",
      options: ["Index 0 (wraps around)", "Index 5 (out of bounds)", "Index 3", "Index 1"],
      correctAnswer: 0,
      explanation: "rear + 1 = 5, but 5 % 5 = 0, so the element is placed at index 0 (circular wrap-around).",
    },
    {
      question: "When is a circular queue considered full?",
      options: ["front == -1", "rear == MAX_SIZE - 1", "(rear + 1) % MAX_SIZE == front", "rear == front"],
      correctAnswer: 2,
      explanation: "A circular queue is full when the next position after rear is front, i.e., (rear + 1) % MAX_SIZE == front.",
    },
    {
      question: "When is a circular queue considered empty?",
      options: ["front == -1", "rear == -1", "front == rear == -1", "rear == 0"],
      correctAnswer: 2,
      explanation: "A circular queue is empty when both front and rear are -1.",
    },
    {
      question: "What is the main disadvantage of a linear (non-circular) array queue?",
      options: ["O(n) dequeue due to shifting", "No random access", "Cannot store duplicate values", "Slow comparisons"],
      correctAnswer: 0,
      explanation: "In a linear array queue, dequeuing requires O(n) shifting of all remaining elements, or wasted space at the front.",
    },
    {
      question: "In a circular queue with MAX_SIZE=5, front=0, rear=3, what is the number of elements?",
      options: ["3", "4", "5", "2"],
      correctAnswer: 1,
      explanation: "Number of elements = rear - front + 1 = 3 - 0 + 1 = 4 elements. If rear was less than front, we'd use (rear + MAX_SIZE - front + 1).",
    },
    {
      question: "What happens if you enqueue to a full circular queue?",
      options: ["Queue Underflow", "Queue Overflow", "Automatic resize", "Oldest element is overwritten"],
      correctAnswer: 1,
      explanation: "Queue Overflow occurs when trying to enqueue to a full circular queue ((rear + 1) % MAX_SIZE == front).",
    },
    {
      question: "Which statement is TRUE about a circular array queue?",
      options: ["It uses less memory than a linked-list queue", "It can wrap around to reuse empty slots", "It always stores elements in sorted order", "It doesn't need front and rear pointers"],
      correctAnswer: 1,
      explanation: "The key feature of a circular queue is reusing empty slots by wrapping around — a simple but effective space optimization.",
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const w = [];
    if (answers[0] !== questions[0].correctAnswer) w.push("front/rear pointers");
    if (answers[1] !== questions[1].correctAnswer) w.push("circular vs linear advantage");
    if (answers[2] !== questions[2].correctAnswer) w.push("time complexity");
    if (answers[3] !== questions[3].correctAnswer) w.push("wrap-around index calculation");
    if (answers[4] !== questions[4].correctAnswer) w.push("full queue condition");
    if (answers[5] !== questions[5].correctAnswer) w.push("empty queue condition");
    if (answers[6] !== questions[6].correctAnswer) w.push("linear queue disadvantages");
    if (answers[7] !== questions[7].correctAnswer) w.push("element count calculation");
    if (answers[8] !== questions[8].correctAnswer) w.push("queue overflow condition");
    if (answers[9] !== questions[9].correctAnswer) w.push("circular queue properties");
    return w.length > 0 ? `Focus on improving: ${w.join(', ')}.` : "Perfect! You've mastered Array Queue!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Array Queue Quiz</h2>
