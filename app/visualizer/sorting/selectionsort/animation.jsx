...
const SelectionSortVisualizer = () => {
  ...
  const getSelectionOptions = (arr, minIndex, j) => {
    const optionLabel = (index) =>
      index >= 0 && index < arr.length ? `Index ${index}: ${arr[index]}` : "No element";

    return {
      question: "Which element is currently minimum?",
      options: [
        optionLabel(minIndex),
        optionLabel(j),
        optionLabel(j - 1),
        optionLabel(j + 1),
      ],
  ...
  const selectionSort = async () => {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      const initialQuestion = getSelectionOptions(arr, minIndex, i + 1);
      await askChallenge(initialQuestion.question, initialQuestion.options, 0);
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        if (arr[j] < arr[minIndex]) {
          ...
          const updateQuestion = getSelectionOptions(arr, minIndex, j);
          await askChallenge(updateQuestion.question, updateQuestion.options, 0);
          if (!isSortingRef.current) return;
        }
          ...
          <div className="col-span-2 bg-gray-100 dark:bg-neutral-900 p-3 rounded mt-2">
            ...
            <div className="text-xs text-gray-500 mt-1">
              {currentStep > 0 && !sorted
                ? `Finding minimum from index ${currentIndices.i}`
                : sorted
                ? "Sorting complete!"
