...
const LcaQuiz = () => {
  const questions = [
    {
      question: "What does LCA stand for in tree algorithms?",
      options: [
      ...
      correctAnswer: 1,
      explanation:
        "LCA stands for Lowest Common Ancestor — the deepest node that is an ancestor of both given nodes.",
    },
    {
      question: "What is the LCA of a node and itself?",
      options: [
        "The root node",
        "The node itself",
      ...
      correctAnswer: 1,
      explanation:
        "The LCA of a node and itself is the node itself, since it is trivially an ancestor of itself.",
    },
    {
      question: "In a tree with root 1, children 2 (left) and 3 (right), where 2 has children 4 and 5, what is the LCA of nodes 4 and 5?",
      options: ["1", "2", "4", "5"],
      correctAnswer: 1,
      explanation:
        "Both nodes 4 and 5 have node 2 as their immediate parent, and 2 is the deepest common ancestor.",
    },
    {
      question: "What is the LCA of the root node and any other node in the tree?",
      options: [
        "The root node",
        "The other node",
      ...
      correctAnswer: 0,
      explanation:
        "The root is an ancestor of every node. Since it is also one of the input nodes, it is the LCA.",
    },
    {
      question: "What is the time complexity of the recursive LCA algorithm?",
      options: ["O(log n)", "O(n)", "O(n²)", "O(1)"],
      correctAnswer: 1,
      explanation:
        "The recursive LCA algorithm visits up to O(n) nodes in the worst case before both target nodes are found.",
    },
    {
      question: "Which advanced technique answers LCA queries in O(log n) time after preprocessing?",
      options: [
        "Binary lifting",
        "Quick sort",
        "Merge sort",
      ...
      correctAnswer: 0,
      explanation:
        "Binary lifting precomputes ancestors at powers of two for each node (O(n log n) preprocessing) and answers LCA queries in O(log n).",
    },
    {
      question: "If two nodes are in different subtrees of the root, what is their LCA?",
      options: [
        "The root",
        "The leftmost node",
        "The deepest node",
      ...
      correctAnswer: 0,
      explanation:
        "If two nodes are in different subtrees of the root, the root is their LCA since it is the only common ancestor.",
    },
    {
      question: "How can LCA be used to compute the distance between two nodes?",
      options: [
        "dist(u, v) = depth(u) + depth(v) - 2 * depth(LCA(u, v))",
        "dist(u, v) = depth(u) + depth(v) + depth(LCA(u, v))",
        "dist(u, v) = depth(LCA(u, v))",
        "dist(u, v) = max(depth(u), depth(v))"
      ],
      correctAnswer: 0,
      explanation:
        "The distance formula uses the LCA: dist(u, v) = depth(u) + depth(v) - 2 * depth(LCA(u, v)).",
    },
    {
      question: "In a BST, how can we find LCA more efficiently?",
      options: [
        "Using the BST property to guide the search",
        "Using binary lifting only",
        "Using level-order traversal",
        "Using prefix sums"
      ],
      correctAnswer: 0,
      explanation:
        "In a BST, we can use the BST property: if both nodes are smaller, go left; if both larger, go right; otherwise, the current node is the LCA.",
    },
    {
      question: "What does the recursive LCA return when one node is an ancestor of the other?",
      options: [
        "The ancestor node (the one higher up)",
        "The descendant node",
        "The root node",
        "Null"
      ],
      correctAnswer: 0,
      explanation:
        "When one node is an ancestor of the other, the recursive algorithm returns the ancestor node, which is the LCA.",
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("LCA definition");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("LCA of a node with itself");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("computing LCA on sample trees");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("LCA involving root");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("recursive LCA time complexity");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("binary lifting technique");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("LCA of nodes in different subtrees");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("distance formula using LCA");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("BST LCA optimization");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("ancestor-descendant LCA case");
    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all LCA concepts!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Lowest Common Ancestor Quiz</h2>
