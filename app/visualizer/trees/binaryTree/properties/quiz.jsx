...
const BinaryTreePropertiesQuiz = () => {
  const questions = [
    {
      question: "What is the maximum number of nodes at level i of a binary tree?",
      options: ["i", "2i", "2^i", "i^2"],
      correctAnswer: 2,
      explanation: "Level i can have at most 2^i nodes. Level 0 has 2^0 = 1 node (root), level 1 has 2^1 = 2 nodes, etc."
    },
    {
      question: "What is the maximum total number of nodes in a binary tree of height h?",
      options: ["2^h", "2^(h+1) - 1", "h^2", "2h + 1"],
      correctAnswer: 1,
      explanation: "A binary tree of height h can have at most 2^(h+1) - 1 nodes. For height 2, max nodes = 2^3 - 1 = 7."
    },
    {
      question: "Which type of binary tree has all nodes with exactly 0 or 2 children?",
      options: ["Complete binary tree", "Full binary tree", "Perfect binary tree", "Skewed binary tree"],
      correctAnswer: 1,
      explanation: "A full (proper) binary tree has every node with either 0 or 2 children. No node has exactly 1 child."
    },
    {
      question: "What is the depth of the root node?",
      options: ["-1", "0", "1", "Depends on tree height"],
      correctAnswer: 1,
      explanation: "The root node has depth 0 because there are 0 edges from the root to itself."
    },
    {
      question: "In a perfect binary tree of height 3, how many nodes are there?",
      options: ["7", "8", "15", "16"],
      correctAnswer: 2,
      explanation: "A perfect binary tree of height 3 has 2^(3+1) - 1 = 2^4 - 1 = 15 nodes."
    },
    {
      question: "What is a leaf node in a binary tree?",
      options: ["The root node", "A node with exactly one child", "A node with no children", "The leftmost node"],
      correctAnswer: 2,
      explanation: "A leaf node (external node) is a node with no children. It has degree 0."
    },
    {
      question: "What is the height of a tree with only a root node?",
      options: ["-1", "0", "1", "undefined"],
      correctAnswer: 1,
      explanation: "The height of a tree with only a root node is 0, as there are 0 edges from the root to the deepest leaf (which is the root itself)."
    },
    {
      question: "Which is true about a complete binary tree?",
      options: [
        "All levels are completely filled",
        "All nodes have exactly 2 children",
        "All levels except possibly the last are filled, and the last level fills from left to right",
        "The tree must be balanced"
      ],
      correctAnswer: 2,
      explanation: "A complete binary tree has all levels completely filled except possibly the last, which fills from left to right with no gaps."
    },
    {
      question: "What is the degree of a node in a binary tree?",
      options: ["The height of the node", "The number of children of the node", "The depth of the node", "The level of the node"],
      correctAnswer: 1,
      explanation: "The degree of a node is the number of children it has. In a binary tree, degree can be 0, 1, or 2."
    },
    {
      question: "In a full binary tree with 7 internal nodes, how many leaf nodes are there?",
      options: ["6", "7", "8", "14"],
      correctAnswer: 2,
      explanation: "In a full binary tree, number of leaf nodes = internal nodes + 1. So with 7 internal nodes, there are 8 leaf nodes."
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("maximum nodes at level i");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("maximum total nodes formula");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("binary tree types (full vs complete vs perfect)");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("depth of root node");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("perfect binary tree node count");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("leaf node definition");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("tree height calculation");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("complete binary tree structure");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("node degree concept");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("full binary tree leaf count");

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join('; ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all binary tree property concepts!";
  };
...
export default BinaryTreePropertiesQuiz;
