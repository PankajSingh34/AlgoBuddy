...
const PostOrderQuiz = () => {
  const questions = [
    {
      question: "What is the order of node visits in post-order traversal?",
      options: [
        "Root → Left → Right",
        "Left → Root → Right",
        "Left → Right → Root",
      ...
      correctAnswer: 2,
      explanation:
        "Post-order traversal follows Left → Right → Root order, visiting children before their parent.",
    },
    {
      question: "In a tree with root 1, left child 2 (with children 4, 5), and right child 3, what is the post-order traversal?",
      options: [
        "[1, 2, 4, 5, 3]",
        "[4, 5, 2, 3, 1]",
        "[1, 2, 3, 4, 5]",
      ...
      correctAnswer: 1,
      explanation:
        "Post-order: Visit left subtree (4, 5, 2), then right subtree (3), then root (1). Result: [4, 5, 2, 3, 1].",
    },
    {
      question: "What is a primary use case for post-order traversal?",
      options: [
        "BST sort (producing sorted order)",
        "Deleting a tree from memory",
        "Prefix expression generation",
        "Level-by-level printing"
      ],
      correctAnswer: 1,
      explanation:
        "Post-order is essential for deleting a tree because children must be deleted before the parent to avoid dangling references.",
    },
    {
      ...
      correctAnswer: 1,
      explanation:
        "Post-order traversal visits each of the n nodes exactly once, resulting in O(n) time complexity.",
    },
    {
      question: "How many stacks are typically needed for iterative post-order traversal?",
      options: ["Zero (it's recursive only)", "One stack", "Two stacks", "Three stacks"],
      correctAnswer: 2,
      explanation:
        "Iterative post-order typically uses two stacks: one for processing and one for storing the result in reverse order.",
    },
    {
      question: "Which traversal of an expression tree produces postfix notation?",
      options: [
        "In-order traversal",
        "Pre-order traversal",
      ...
      correctAnswer: 2,
      explanation:
        "Post-order traversal of an expression tree produces postfix notation (operator after operands), e.g., AB+ instead of A+B.",
    },
    {
      question: "In a post-order traversal, when is the root node visited?",
      options: [
        "First",
        "Second (after left subtree)",
        "Last (after both subtrees)",
        "It depends on the tree structure"
      ],
      correctAnswer: 2,
      explanation:
        "In post-order traversal, the root is visited last — after both the left and right subtrees have been completely traversed.",
    },
    {
      question: "What is the space complexity of recursive post-order traversal?",
      options: ["O(1)", "O(log n)", "O(n)", "O(h) where h is tree height"],
      correctAnswer: 3,
      explanation:
        "The recursive approach uses the call stack, which can go as deep as the tree height h, giving O(h) space.",
    },
    {
      question: "Which of the following is NOT a characteristic of post-order traversal?",
      options: [
        "Children are visited before their parent",
        "It is useful for tree deletion",
        "It produces sorted output from a BST",
        "It evaluates postfix expressions"
      ],
      correctAnswer: 2,
      explanation:
        "In-order traversal, not post-order, produces sorted output from a BST. Post-order visits children before parents.",
    },
    {
      question: "What happens if you run post-order on a tree with only a root node?",
      options: [
        "It returns an empty sequence",
        "It visits the root node once",
        "It causes a stack overflow",
        "It visits the root twice"
      ],
      correctAnswer: 1,
      explanation:
        "With only a root node, post-order visits the root (no children to process first). The result is a single-element sequence.",
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("post-order traversal order definition");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("computing post-order on sample trees");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("applications of post-order traversal");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("time complexity analysis");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("iterative two-stack implementation");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("postfix expression evaluation");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("root visit timing in post-order");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("space complexity analysis");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("post-order vs in-order characteristics");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("edge cases (single node)");
    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all post-order traversal concepts!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Post-Order Traversal Quiz</h2>
