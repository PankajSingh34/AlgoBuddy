...
export default function PostOrderVisualizer() {
  ...
  const postOrderTraversal = (node, path = []) => {
    if (!node) return path;
    const leftPath = postOrderTraversal(node.left, path);
    const rightPath = postOrderTraversal(node.right, leftPath);
    rightPath.push({ value: node.value, action: 'visit', highlighted: true });
    return rightPath;
  };

  const visualizePostOrder = () => {
    if (!root) {
      setMessage('Tree is empty!');
      return;
    }
    setIsAnimating(true);
    setMessage('Performing post-order traversal...');
    setTraversalResult([]);
    setHighlightedNodes([]);
    setSteps(0);

    const traversalPath = postOrderTraversal(root);
    let step = 0;
    ...
    const animateStep = () => {
      if (step < traversalPath.length) {
        ...
        setMessage(`Post-order traversal complete: [${traversalPath.map(n => n.value).join(', ')}]`);
        setIsAnimating(false);
        setHighlightedNodes([]);
      }
    };
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-2">
            <span className="text-blue-600 dark:text-blue-600">Post-Order Traversal</span> Visualizer
          </h1>
          <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            Visualize how post-order traversal visits nodes in a binary search tree
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div className="flex flex-col gap-2">
              <button onClick={visualizePostOrder} disabled={!root || isAnimating}
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold mb-3">About Post-Order Traversal</h2>
          <div className="space-y-4">
            <div className="prose dark:prose-invert text-sm">
              ...
              <ol className="pl-5 space-y-1">
                <li>Left subtree</li>
                <li>Right subtree</li>
                <li>Root node (visit last)</li>
              </ol>
              <p className="mt-2">The node is processed after its children, making it useful for tree deletion.</p>
            </div>
