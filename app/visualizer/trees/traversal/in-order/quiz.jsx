...
const InOrderQuiz = () => {
  const questions = [
    {
      question: "What is the order of node visits in in-order traversal?",
      options: [
        "Root → Left → Right",
        "Left → Root → Right",
        "Left → Right → Root",
      ...
      correctAnswer: 1,
      explanation:
        "In-order traversal follows Left → Root → Right order, visiting the left subtree first, then the root, then the right subtree.",
    },
    {
      question: "What property does in-order traversal have when applied to a Binary Search Tree (BST)?",
      options: [
        "Produces nodes in random order",
        "Produces nodes in reverse sorted order",
        "Produces nodes in ascending sorted order",
        "Produces nodes in level order"
      ],
      correctAnswer: 2,
      explanation:
        "In-order traversal of a BST produces nodes in ascending sorted order because of the BST property (left < root < right).",
    },
    {
      ...
      correctAnswer: 1,
      explanation:
        "In-order traversal visits each of the n nodes exactly once, resulting in O(n) time complexity.",
    },
    {
      question: "What is the space complexity of recursive in-order traversal?",
      options: ["O(1)", "O(n)", "O(log n)", "O(h) where h is tree height"],
      correctAnswer: 3,
      explanation:
        "The recursive approach uses the call stack, which can go as deep as the tree height h, giving O(h) space.",
    },
    {
      question: "For an empty tree, what does in-order traversal produce?",
      options: [
        "An error",
        "A single null value",
        "An empty sequence",
      ...
      correctAnswer: 2,
      explanation:
        "In-order traversal of an empty tree produces an empty sequence (no nodes to visit).",
    },
    {
      question: "Which data structure is used to implement iterative in-order traversal?",
      options: [
        "Queue",
        "Stack",
        "Array",
      ...
      correctAnswer: 1,
      explanation:
        "Iterative in-order traversal uses an explicit stack to simulate the recursion call stack, storing nodes that need to be revisited.",
    },
    {
      question: "Which of the following is NOT a real-world application of in-order traversal?",
      options: [
        "BST sort (producing sorted output)",
      ...
      correctAnswer: 2,
      explanation:
        "Finding shortest paths uses graph algorithms like Dijkstra's or BFS, not tree traversal. In-order is used for BST sort, expression trees, and BST validation.",
    },
    {
      question: "Given a BST with root 8, left subtree containing 3→1, right subtree containing 10→14, what is the in-order sequence?",
      options: [
        "[8, 3, 1, 10, 14]",
        "[1, 3, 8, 10, 14]",
        "[14, 10, 8, 3, 1]",
      ...
      correctAnswer: 1,
      explanation:
        "In-order traversal: visit leftmost (1), then its parent (3), then root (8), then right subtree leftmost (10), then its right (14). Result: [1, 3, 8, 10, 14].",
    },
    {
      question: "What happens if you perform in-order traversal on a degenerate (skewed) tree with n nodes?",
      options: [
        "It visits all nodes but uses O(n) stack space recursively",
        "It fails due to stack overflow",
        "It visits only leaf nodes",
        "It runs in O(log n) time"
      ],
      correctAnswer: 0,
      explanation:
        "A degenerate tree has height n, so recursive in-order uses O(n) stack space. It still visits all n nodes in O(n) time.",
    },
    {
      question: "Which traversal order is used to get elements of a BST in descending order?",
      options: [
      ...
      correctAnswer: 1,
      explanation:
        "Traversing in Right → Root → Left (reverse in-order) visits nodes in descending order, the reverse of standard in-order.",
    },
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("in-order traversal order definition");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("BST in-order property (sorted output)");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("time complexity analysis");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("space complexity (recursive vs iterative)");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("edge cases (empty tree)");
    }
    if (answers[5] !== questions[5].correctAnswer) {
      weakAreas.push("iterative implementation details");
    }
    if (answers[6] !== questions[6].correctAnswer) {
      weakAreas.push("applications of in-order traversal");
    }
    if (answers[7] !== questions[7].correctAnswer) {
      weakAreas.push("computing in-order on sample trees");
    }
    if (answers[8] !== questions[8].correctAnswer) {
      weakAreas.push("degenerate tree behavior");
    }
    if (answers[9] !== questions[9].correctAnswer) {
      weakAreas.push("reverse in-order for descending order");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all in-order traversal concepts!";
  };
