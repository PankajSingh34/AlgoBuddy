...
const PreOrderQuiz = () => {
  const questions = [
    {
      question: "What is the order of node visits in pre-order traversal?",
      options: [
        "Root → Left → Right",
        "Left → Root → Right",
        "Left → Right → Root",
      ...
      correctAnswer: 0,
      explanation:
        "Pre-order traversal follows Root → Left → Right order, visiting the root first, then the left subtree, then the right subtree.",
    },
    {
      question: "In a tree with root 1, left child 2 (with children 4, 5), and right child 3, what is the pre-order traversal?",
      options: [
        "[4, 5, 2, 3, 1]",
        "[1, 2, 4, 5, 3]",
        "[1, 2, 3, 4, 5]",
      ...
      correctAnswer: 1,
      explanation:
        "Pre-order: Visit root 1, then left subtree (visit 2, then 4, then 5), then right subtree (visit 3). Result: [1, 2, 4, 5, 3].",
    },
    {
      question: "What is a common application of pre-order traversal?",
      options: [
      ...
      correctAnswer: 1,
      explanation:
        "Pre-order is used for tree serialization because the root is written first, allowing the tree structure to be reconstructed from the serialized data.",
    },
    {
      ...
      correctAnswer: 1,
      explanation:
        "Pre-order traversal visits each of the n nodes exactly once, resulting in O(n) time complexity.",
    },
    {
      question: "Which data structure is used for iterative pre-order traversal?",
      options: ["Queue", "Stack", "Array", "Heap"],
      correctAnswer: 1,
      explanation:
        "Iterative pre-order traversal uses a stack. Push the root, then pop and visit, pushing right then left child.",
    },
    {
      question: "In the iterative version of pre-order traversal, which child is pushed onto the stack first?",
      options: [
        "Left child first",
        "Right child first",
      ...
      correctAnswer: 1,
      explanation:
        "In iterative pre-order, we push the right child first, then the left child, so the left child is popped next (LIFO order ensures left is processed before right).",
    },
    {
      question: "Which of the following is NOT a use case for pre-order traversal?",
      options: [
      ...
      correctAnswer: 2,
      explanation:
        "BST sort uses in-order traversal, not pre-order. Pre-order is used for prefix expressions, serialization, and syntax tree traversal.",
    },
    {
      question: "For an empty tree, what does pre-order traversal return?",
      options: [
        "An error",
        "A single null value",
        "An empty sequence",
      ...
      correctAnswer: 2,
      explanation:
        "Pre-order traversal of an empty tree returns an empty sequence, as there are no nodes to visit.",
    },
    {
      question: "How does the recursion stack grow during pre-order traversal of a degenerate (skewed) tree?",
      options: [
        "It stays constant at O(1)",
        "It grows to O(log n)",
        "It grows to O(n)",
      ...
      correctAnswer: 2,
      explanation:
        "In a degenerate tree (height = n), the recursion stack depth equals the height, requiring O(n) space for recursive pre-order traversal.",
    },
    {
      question: "Which traversal produces prefix expressions from an expression tree?",
      options: [
        "In-order traversal",
        "Pre-order traversal",
      ...
      correctAnswer: 1,
      explanation:
        "Pre-order traversal of an expression tree produces prefix notation (operator before operands), e.g., +AB instead of A+B.",
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("pre-order traversal order definition");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("computing pre-order on sample trees");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("applications of pre-order traversal");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("time complexity analysis");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("iterative stack-based implementation");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("stack push order in iterative pre-order");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("pre-order vs other traversals");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("edge cases (empty tree)");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("recursion stack behavior");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("prefix expression generation");
    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all pre-order traversal concepts!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Pre-Order Traversal Quiz</h2>
