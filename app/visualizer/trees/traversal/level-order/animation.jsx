...
export default function LevelOrderVisualizer() {
  ...
  const levelOrderTraversal = (root) => {
    ...
    const path = [];
    const queue = [root];
    const queueSnapshots = [[...queue.map(n => n.value)]];

    while (queue.length > 0) {
      const current = queue.shift();
      path.push({ value: current.value, action: 'visit', highlighted: true });
      if (current.left) queue.push(current.left);
      if (current.right) queue.push(current.right);
      queueSnapshots.push([...queue.map(n => n.value)]);
    }
    return { path, queueSnapshots };
  };

  const visualizeLevelOrder = () => {
    if (!root) {
      setMessage('Tree is empty!');
      return;
    }
    setIsAnimating(true);
    setMessage('Performing level-order traversal...');
    setTraversalResult([]);
    setHighlightedNodes([]);
    setSteps(0);
    setQueueState([]);

    const { path, queueSnapshots } = levelOrderTraversal(root);
    let step = 0;
    ...
    const animateStep = () => {
      if (step < path.length) {
        ...
        setQueueState(queueSnapshots[step + 1] || []);
        setSteps(step + 1);
        step++;
        animationRef.current = setTimeout(animateStep, 1000 / speed);
      } else {
        setMessage(`Level-order traversal complete: [${path.map(n => n.value).join(', ')}]`);
        setIsAnimating(false);
        setHighlightedNodes([]);
        setQueueState([]);
      }
  ...
  const renderTree = (node, x = 400, y = 50, level = 0, nodes = [], edges = []) => {
    ...
    const yOffset = 80;
    nodes.push({ value: node.value, x, y, highlighted: highlightedNodes.includes(node.value) });
    if (node.left) {
      edges.push({ x1: x, y1: y + nodeRadius, x2: x - xOffset, y2: y + yOffset - nodeRadius });
      renderTree(node.left, x - xOffset, y + yOffset, level + 1, nodes, edges);
    }
    if (node.right) {
      edges.push({ x1: x, y1: y + nodeRadius, x2: x + xOffset, y2: y + yOffset - nodeRadius });
      renderTree(node.right, x + xOffset, y + yOffset, level + 1, nodes, edges);
    }
    return { nodes, edges };
  };
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-2">
            <span className="text-blue-600 dark:text-blue-600">Level-Order Traversal</span> Visualizer
          </h1>
          <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            Visualize how level-order traversal visits nodes using a queue
          </p>
        </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input type="number" value={inputValue} onChange={(e) => setInputValue(e.target.value)}
                  ...
                  disabled={isAnimating} onKeyPress={(e) => e.key === 'Enter' && handleInsert()} />
                <button onClick={handleInsert} disabled={isAnimating}
                  className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg disabled:opacity-50 transition-colors">Insert</button>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={visualizeLevelOrder} disabled={!root || isAnimating}
              ...
              <button onClick={reset}
                className="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-colors">Reset All</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-3">
              ...
              <input type="range" min="0.5" max="5" step="0.5" value={speed}
                onChange={(e) => setSpeed(parseFloat(e.target.value))} className="flex-1" disabled={isAnimating} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {nodes.length > 0 ? (
                <div className="relative" style={{ minWidth: `${svgDimensions.width}px` }}>
                  <svg width={svgDimensions.width} height={svgDimensions.height}
                    viewBox={`0 0 ${svgDimensions.width} ${svgDimensions.height}`} className="mx-auto">
                    {edges.map((edge, i) => (
                    ...
                    {nodes.map((node, i) => (
                      <g key={i}>
                        <circle cx={node.x} cy={node.y} r="22"
                          ...
                          stroke={node.highlighted ? "#d97706" : "#1d4ed8"} strokeWidth="2"
                          className={`transition-colors ${node.highlighted ? "animate-pulse" : ""}`} />
                        <text x={node.x} y={node.y + 5} textAnchor="middle" fill="white" fontSize="13" fontWeight="600">
                          {node.value}
                        </text>
                      </g>
                    ))}
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-3">Queue State</h2>
            <div className="min-h-[200px] flex flex-col items-center justify-start space-y-2">
              {queueState.length > 0 ? (
                queueState.map((val, i) => (
                  <div key={i}
                    className="w-full p-2 bg-blue-100 dark:bg-blue-900/40 border border-blue-300 dark:border-blue-700 rounded text-center font-mono font-bold text-blue-800 dark:text-blue-200 animate-pulse">
                    {val}
                  </div>
                ))
              ) : (
                <div className="text-gray-500 dark:text-gray-400 text-sm italic mt-8">
                  {isAnimating ? "Queue empty" : "Run traversal to see queue"}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mt-6 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold mb-3">About Level-Order Traversal</h2>
          <div className="space-y-4">
            <div className="prose dark:prose-invert text-sm">
              <p>Visits nodes level by level from top to bottom using a queue.</p>
              <ol className="pl-5 space-y-1">
                <li>Enqueue root node</li>
                <li>Dequeue and visit node</li>
                <li>Enqueue left child, then right child</li>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="bg-gray-100 dark:bg-gray-700 p-2 rounded-lg text-center">
                ...
                <div className="font-bold">O(w)</div>
              </div>
            </div>
          </div>
        </div>
