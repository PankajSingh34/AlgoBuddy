...
const LevelOrderQuiz = () => {
  const questions = [
    {
      question: "What data structure is primarily used for level-order traversal?",
      options: ["Stack", "Queue", "Array", "Heap"],
      correctAnswer: 1,
      explanation:
        "Level-order traversal uses a queue to maintain the order of nodes to be visited, processing nodes in FIFO order.",
    },
    {
      question: "What is another name for level-order traversal?",
      options: [
      ...
      correctAnswer: 1,
      explanation:
        "Level-order traversal is also known as Breadth-First Search (BFS) for trees, visiting nodes level by level.",
    },
    {
      question: "In a tree with root 1, left child 2 (with children 4, 5), and right child 3, what is the level-order traversal?",
      options: [
        "[1, 2, 4, 5, 3]",
        "[1, 2, 3, 4, 5]",
        "[4, 5, 2, 3, 1]",
      ...
      correctAnswer: 1,
      explanation:
        "Level-order: Level 0: [1], Level 1: [2, 3], Level 2: [4, 5]. Result: [1, 2, 3, 4, 5].",
    },
    {
      question: "What is the worst-case space complexity of level-order traversal?",
      options: ["O(1)", "O(log n)", "O(n)", "O(h)"],
      correctAnswer: 2,
      explanation:
        "In the worst case (a perfect binary tree), the queue can hold up to n/2 nodes (all leaf nodes at the last level), giving O(n) space.",
    },
    {
      question: "Which of the following is NOT typically solved using level-order traversal?",
      options: [
        "Finding the minimum depth of a tree",
        "Finding the right-side view of a binary tree",
        "BST sort (producing sorted order)",
        "Serializing a tree level by level"
      ],
      correctAnswer: 2,
      explanation:
        "BST sort uses in-order traversal, not level-order. Level-order is used for minimum depth, right-side view, and serialization.",
    },
    {
      ...
      correctAnswer: 1,
      explanation:
        "Level-order traversal visits each of the n nodes exactly once, resulting in O(n) time complexity.",
    },
    {
      question: "In level-order traversal, what signals that the traversal is complete?",
      options: [
        "The root is revisited",
        "The queue becomes empty",
        "The stack overflows",
        "All leaves have been visited"
      ],
      correctAnswer: 1,
      explanation:
        "Level-order traversal is complete when the queue is empty, meaning there are no more nodes to process.",
    },
    {
      question: "What happens to a node in level-order traversal when it is dequeued?",
      options: [
        "It is discarded",
        "It is visited and its children are enqueued",
        "It is pushed onto a stack",
        "It is marked as deleted"
      ],
      correctAnswer: 1,
      explanation:
        "When a node is dequeued, it is visited (processed), and then its left and right children are enqueued for later processing.",
    },
    {
      question: "How does level-order traversal differ from DFS traversals?",
      options: [
        "It uses less memory in all cases",
        "It visits nodes level by level rather than going deep first",
        "It only works on complete binary trees",
        "It cannot be implemented recursively"
      ],
      correctAnswer: 1,
      explanation:
        "Level-order (BFS) visits all nodes at the current depth before moving deeper, while DFS traversals go deep before visiting siblings.",
    },
    {
      question: "What is the maximum queue size during level-order traversal of a perfect binary tree of height h?",
      options: ["h", "2h", "2^h (leaf level)", "h²"],
      correctAnswer: 2,
      explanation:
        "At the leaf level of a perfect binary tree, there are 2^h nodes (where h is height), which is also approximately n/2 for the entire tree.",
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("queue usage in level-order");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("BFS vs DFS terminology");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("computing level-order on sample trees");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("space complexity (queue size)");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("applications of level-order");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("time complexity analysis");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("queue empty condition");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("dequeue and enqueue process");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("BFS vs DFS comparison");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("maximum queue size analysis");
    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered all level-order traversal concepts!";
  };
  ...
  const getStarRating = () => {
    ...
    if (p >= 90) return 5; if (p >= 70) return 4; if (p >= 50) return 3; if (p >= 30) return 2; return 1;
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">Level-Order Traversal Quiz</h2>
