...
const BSTSearchingQuiz = () => {
  const questions = [
    {
      question: "What is the first step when searching for a value in a BST?",
      options: ["Search the left subtree first", "Compare target with the root", "Search the right subtree first", "Sort the tree first"],
      correctAnswer: 1,
      explanation: "Search always starts by comparing the target value with the root node."
    },
    {
      question: "If the target value is smaller than the current node, where do we go?",
      options: ["Right subtree", "Left subtree", "Back to the parent", "Nowhere — search ends"],
      correctAnswer: 1,
      explanation: "In a BST, smaller values are always in the left subtree, so we move left."
    },
    {
      question: "What is the time complexity of BST search in a balanced tree?",
      options: ["O(1)", "O(log n)", "O(n)", "O(n²)"],
      correctAnswer: 1,
      explanation: "In a balanced BST, search takes O(log n) since the height h = O(log n)."
    },
    {
      question: "In the worst case, what is the time complexity of BST search?",
      options: ["O(1)", "O(log n)", "O(n)", "O(n log n)"],
      correctAnswer: 2,
      explanation: "In a skewed tree (height = n), search degrades to O(n) — essentially linear search."
    },
    {
      question: "When does BST search have its best-case performance?",
      options: ["When the tree is balanced", "When the target is at the root", "When the tree is full", "When the target is a leaf"],
      correctAnswer: 1,
      explanation: "Best case O(1) occurs when the target value is at the root — only one comparison needed."
    },
    {
      question: "What does BST search return if the target is not in the tree?",
      options: ["The parent node", "The closest value", "null or 'not found'", "The root node"],
      correctAnswer: 2,
      explanation: "If the search reaches a null child, the value is not present, and null/not-found is returned."
    },
    {
      question: "How many comparisons to find 6 in BST [8, 3, 10, 1, 6, 14]?",
      options: ["1", "2", "3", "4"],
      correctAnswer: 2,
      explanation: "Compare with 8 (go left), compare with 3 (go right), compare with 6 (found). Three comparisons."
    },
    {
      question: "Which approach can be used to implement BST search?",
      options: ["Only recursive", "Only iterative", "Both recursive and iterative", "Only using arrays"],
      correctAnswer: 2,
      explanation: "BST search can be implemented both recursively (elegant) and iteratively (avoids stack overflow)."
    },
    {
      question: "Why does BST search eliminate half the tree at each step in a balanced tree?",
      options: ["The tree is sorted by size", "The BST property guarantees one subtree can be skipped", "The search algorithm is random", "The tree uses hashing"],
      correctAnswer: 1,
      explanation: "Due to the BST property, if the target is smaller than the root, the entire right subtree is guaranteed not to contain it."
    },
    {
      question: "How is BST search different from Binary Search on an array?",
      options: ["They are exactly the same", "BST search traverses nodes; Binary Search uses array indices", "BST is faster", "Binary Search is faster"],
      correctAnswer: 1,
      explanation: "BST search follows pointers from node to node, while Binary Search on arrays uses index calculations to find the middle element."
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("starting point of BST search");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("left vs right traversal rules");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("balanced tree time complexity");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("worst-case complexity");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("best-case scenario");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("not-found return value");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("comparison counting");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("recursive vs iterative approaches");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("subtree elimination property");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("BST vs Binary Search comparison");
    return weakAreas.length > 0 ? `Focus on improving: ${weakAreas.join('; ')}.` : "Perfect! You've mastered BST searching!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">BST Searching Quiz</h2>
...
export default BSTSearchingQuiz;
