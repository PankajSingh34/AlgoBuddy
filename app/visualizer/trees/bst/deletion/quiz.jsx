...
const BSTDeletionQuiz = () => {
  const questions = [
    {
      question: "How many deletion cases exist in a BST?",
      options: ["1", "2", "3", "4"],
      correctAnswer: 2,
      explanation: "BST deletion has 3 cases: leaf (0 children), one child, and two children."
    },
    {
      question: "What happens when deleting a leaf node from a BST?",
      options: ["Replace it with its left child", "Simply remove it (set parent's pointer to null)", "Replace it with its right child", "Find inorder successor"],
      correctAnswer: 1,
      explanation: "A leaf node has no children, so we simply remove it by setting the parent's pointer to null."
    },
    {
      question: "What is the first step when deleting a node with two children?",
      options: ["Remove the node directly", "Find the inorder successor", "Replace with left child", "Rotate the tree"],
      correctAnswer: 1,
      explanation: "For a two-child deletion, we first find the inorder successor (smallest in right subtree) to replace the node's value."
    },
    {
      question: "What is an inorder successor of a node in a BST?",
      options: ["The largest node in the left subtree", "The smallest node in the right subtree", "The parent node", "The left child"],
      correctAnswer: 1,
      explanation: "The inorder successor is the smallest node in the right subtree — found by going right once, then left repeatedly."
    },
    {
      question: "After finding the inorder successor, what do we do?",
      options: ["Delete the successor node directly", "Copy the successor's value to the target node, then delete the successor", "Swap the successor with the target node's parent", "Rotate the tree around the successor"],
      correctAnswer: 1,
      explanation: "We copy the successor's value into the target node, then delete the successor (which now has at most one child)."
    },
    {
      question: "What is the time complexity of BST deletion in a balanced tree?",
      options: ["O(1)", "O(log n)", "O(n)", "O(n²)"],
      correctAnswer: 1,
      explanation: "In a balanced BST, deletion takes O(log n) time since we traverse from root to the target and potentially find a successor."
    },
    {
      question: "If a node has only a left child, how is it deleted?",
      options: ["Set parent's pointer to the left child", "Set parent's pointer to null", "Find inorder successor", "Copy left child value to target"],
      correctAnswer: 0,
      explanation: "If a node has one child, we bypass it by setting the parent's pointer to that child."
    },
    {
      question: "What happens to the BST property during deletion of a node with two children?",
      options: ["It is temporarily violated and fixed by rotation", "It is preserved by replacing with inorder successor", "The tree must be rebuilt", "It is violated permanently"],
      correctAnswer: 1,
      explanation: "The inorder successor is the smallest value larger than the target, so placing it at the target's position preserves BST ordering."
    },
    {
      question: "In the tree [8, 3, 10, 1, 6, 14], what is the inorder successor of 8?",
      options: ["3", "6", "10", "14"],
      correctAnswer: 2,
      explanation: "The inorder successor of 8 is the smallest value in its right subtree, which is 10 (go right once to 10, then leftmost)."
    },
    {
      question: "Can a node with two children be deleted by finding the inorder predecessor instead?",
      options: ["No, only successor works", "Yes, the largest in the left subtree also works", "Only in balanced trees", "Only in AVL trees"],
      correctAnswer: 1,
      explanation: "Yes, the inorder predecessor (largest in left subtree) works just as well as the successor for the two-child case."
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("number of deletion cases");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("leaf node deletion");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("two-child deletion procedure");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("inorder successor definition");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("successor value copy process");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("time complexity");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("one-child deletion");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("BST property preservation");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("inorder successor identification");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("inorder predecessor alternative");

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join('; ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered BST deletion!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">BST Deletion Quiz</h2>
        ...
        <div className="flex flex-col items-center justify-center h-64">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 500, damping: 15 }} className="mb-4">
            <div className="relative">
              <div className="w-24 h-24 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center"><FaCheck className="text-4xl text-green-500 animate-bounce" /></div>
...
export default BSTDeletionQuiz;
