...
const AVLQuiz = () => {
  const questions = [
    {
      question: "What is a balance factor in an AVL tree?",
      options: ["Height of left subtree", "Height of right subtree", "Height(L) - Height(R)", "Height(L) + Height(R)"],
      correctAnswer: 2,
      explanation: "Balance factor = height(left subtree) - height(right subtree). In AVL, it must be -1, 0, or 1."
    },
    {
      question: "What rotation corrects a Left-Left (LL) imbalance?",
      options: ["Left rotation", "Right rotation", "Left-Right rotation", "Right-Left rotation"],
      correctAnswer: 1,
      explanation: "LL imbalance is fixed with a right rotation (also called a right rotate on the unbalanced node)."
    },
    {
      question: "When does a Left-Right (LR) rotation apply?",
      options: ["Left child's right subtree is too deep", "Right child's left subtree is too deep", "Left child's left subtree is too deep", "Right child's right subtree is too deep"],
      correctAnswer: 0,
      explanation: "LR rotation applies when the left child's right subtree causes the imbalance — first left rotate the child, then right rotate the node."
    },
    {
      question: "What is the maximum allowed balance factor in an AVL tree?",
      options: ["0", "1", "2", "3"],
      correctAnswer: 1,
      explanation: "The balance factor must be -1, 0, or 1. An absolute value of 2 or more means the tree is unbalanced and needs rotation."
    },
    {
      question: "Who invented the AVL tree?",
      options: ["Dijkstra and Floyd", "Adelson-Velsky and Landis", "Knuth and Morris", "Hoare and Bayer"],
      correctAnswer: 1,
      explanation: "AVL stands for Adelson-Velsky and Landis, the Soviet mathematicians who invented it in 1962."
    },
    {
      question: "What is the guaranteed time complexity of search in an AVL tree?",
      options: ["O(n)", "O(log n)", "O(1)", "O(n log n)"],
      correctAnswer: 1,
      explanation: "AVL trees guarantee O(log n) search time because the self-balancing property keeps the height logarithmic."
    },
    {
      question: "How tight is the height bound of an AVL tree?",
      options: ["1.44 * log2(n)", "2 * log2(n)", "log2(n)", "n"],
      correctAnswer: 0,
      explanation: "The height of an AVL tree is bounded by approximately 1.44 * log2(n), making it very close to optimal."
    },
    {
      question: "What rotation sequence fixes an RL (Right-Left) imbalance?",
      options: ["Left then right", "Right then left", "Left only", "Right only"],
      correctAnswer: 1,
      explanation: "RL imbalance is fixed by first doing a right rotation on the right child, then a left rotation on the unbalanced node."
    },
    {
      question: "After an insertion in an AVL tree, how many rotations are needed at most?",
      options: ["O(log n)", "At most 1 or 2", "O(n)", "None"],
      correctAnswer: 1,
      explanation: "AVL insertion requires at most one or two rotations (single or double rotation) to restore balance."
    },
    {
      question: "What is the balance factor requirement for every node in an AVL tree?",
      options: ["Always 0", "Always positive", "Between -1 and 1 inclusive", "Between -2 and 2 inclusive"],
      correctAnswer: 2,
      explanation: "Every node in an AVL tree must have a balance factor of -1, 0, or 1. This guarantees the tree is balanced."
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("balance factor definition");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("LL rotation");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("LR rotation conditions");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("balance factor limits");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("AVL history/inventors");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("guaranteed time complexity");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("height bound");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("RL rotation sequence");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("number of rotations per insertion");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("balance factor range requirement");
    return weakAreas.length > 0 ? `Focus on improving: ${weakAreas.join('; ')}.` : "Perfect! You've mastered AVL trees!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">AVL Tree Balancing Quiz</h2>
