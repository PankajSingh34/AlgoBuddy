...
const BSTInsertionQuiz = () => {
  const questions = [
    {
      question: "In a BST, where does a new node with a smaller value go?",
      options: ["Right subtree", "Left subtree", "Directly as root", "Nowhere — duplicates not allowed"],
      correctAnswer: 1,
      explanation: "In a BST, nodes with smaller values are placed in the left subtree to maintain the BST ordering property."
    },
    {
      question: "What is the time complexity of BST insertion in a balanced tree?",
      options: ["O(1)", "O(log n)", "O(n)", "O(n²)"],
      correctAnswer: 1,
      explanation: "In a balanced BST with n nodes, height h = O(log n), so insertion takes O(log n) time."
    },
    {
      question: "What happens when inserting the value 6 into a BST with root 5 and right child 7?",
      options: ["6 becomes left child of 7", "6 becomes right child of 7", "6 replaces 5 as root", "6 becomes left child of 5"],
      correctAnswer: 0,
      explanation: "6 > 5 so go right to 7, then 6 < 7 so 6 becomes the left child of 7."
    },
    {
      question: "What is the worst-case time complexity of BST insertion?",
      options: ["O(1)", "O(log n)", "O(n)", "O(n log n)"],
      correctAnswer: 2,
      explanation: "In a skewed tree (e.g., inserting sorted data), height h = O(n), so insertion degrades to O(n)."
    },
    {
      question: "After inserting 1, 2, 3, 4, 5 into an empty BST in order, what shape is the tree?",
      options: ["Perfectly balanced", "Complete binary tree", "Skewed (like a linked list)", "Full binary tree"],
      correctAnswer: 2,
      explanation: "Inserting sorted values creates a skewed tree. 1 is root, 2 is right of 1, 3 is right of 2, etc."
    },
    {
      question: "When inserting a duplicate value into a BST that uses right-subtree convention, where does it go?",
      options: ["Left subtree", "Right subtree", "It replaces the existing node", "It is rejected"],
      correctAnswer: 1,
      explanation: "With right-subtree convention for duplicates, equal values go to the right subtree."
    },
    {
      question: "What is the first step of BST insertion?",
      options: ["Create a new node", "Find the correct parent position", "Compare with root", "If tree is empty, insert as root"],
      correctAnswer: 3,
      explanation: "The first step is checking if the tree is empty. If so, the new node becomes the root."
    },
    {
      question: "How many comparisons are needed to insert 9 into a BST containing [5, 3, 7, 2, 4, 6, 8]?",
      options: ["1", "2", "3", "4"],
      correctAnswer: 2,
      explanation: "9 > 5 (go right), 9 > 7 (go right), 9 > 8 (go right, insert as right child of 8). Three comparisons."
    },
    {
      question: "What property does BST insertion maintain?",
      options: ["The tree must be balanced", "The BST ordering property", "The tree must be complete", "All leaves at same level"],
      correctAnswer: 1,
      explanation: "BST insertion maintains the BST ordering property: left < root < right at every node."
    },
    {
      question: "Which data structure helps avoid O(n) insertion time in worst-case BST?",
      options: ["Heap", "Self-balancing BST (AVL, Red-Black)", "Hash table", "Linked list"],
      correctAnswer: 1,
      explanation: "Self-balancing BSTs like AVL and Red-Black trees use rotations to maintain O(log n) height after insertions."
    },
  ];
  ...
  const calculateWeakAreas = () => {
    const weakAreas = [];
    if (answers[0] !== questions[0].correctAnswer) weakAreas.push("BST left/right placement rule");
    if (answers[1] !== questions[1].correctAnswer) weakAreas.push("balanced BST time complexity");
    if (answers[2] !== questions[2].correctAnswer) weakAreas.push("insertion path traversal");
    if (answers[3] !== questions[3].correctAnswer) weakAreas.push("worst-case complexity");
    if (answers[4] !== questions[4].correctAnswer) weakAreas.push("skewed tree from sorted insertion");
    if (answers[5] !== questions[5].correctAnswer) weakAreas.push("duplicate handling");
    if (answers[6] !== questions[6].correctAnswer) weakAreas.push("insertion algorithm steps");
    if (answers[7] !== questions[7].correctAnswer) weakAreas.push("comparison counting");
    if (answers[8] !== questions[8].correctAnswer) weakAreas.push("BST ordering property");
    if (answers[9] !== questions[9].correctAnswer) weakAreas.push("self-balancing trees");

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join('; ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered BST insertion!";
  };
    ...
    <section className="max-w-4xl mx-auto shadow-lg rounded-xl bg-white dark:bg-neutral-950 mt-8 mb-8 p-6 border border-gray-200 dark:border-gray-700">
      {showIntro ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4 text-black dark:text-gray-100">BST Insertion Quiz</h2>
        ...
        <div>
          ...
          <div className="flex justify-between">
            <button onClick={handlePreviousQuestion} disabled={currentQuestion === 0} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-lg disabled:opacity-50 flex items-center hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
              <FaArrowLeft className="mr-2" /> Previous
            </button>
            <button onClick={handleNextQuestion} disabled={selectedAnswer === null} className="px-4 py-2 bg-blue-600 text-white rounded-lg disabled:opacity-50 hover:from-blue-600 hover:to-purple-600 transition-all flex items-center shadow-md hover:shadow-lg">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="text-center mb-8">
            <div className="relative inline-block mb-4">
              ...
              <div className="absolute -bottom-2 left-0 right-0 flex justify-center">
                {[...Array(5)].map((_, i) => (
                  <FaStar key={i} className={`text-2xl mx-1 ${i < getStarRating() ? 'text-yellow-400 fill-current' : 'text-gray-300 dark:text-gray-600 fill-current'}`} />
                ))}
              </div>
            </div>
          ...
          <button onClick={resetQuiz} className="w-full px-6 py-3 bg-blue-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-600 transition-all shadow-md hover:shadow-lg flex items-center justify-center">
...
export default BSTInsertionQuiz;
