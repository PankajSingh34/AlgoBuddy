...
import { editDistanceGenerator } from "@/features/algorithms/dp/editDistanceLogic";
...
export default function DPVisualizer({ algorithm = "knapsack" }) {
  ...
  // Inputs for Edit Distance
  const [edStr1, setEdStr1] = useState(topic.defaultInput.str1 || "");
  const [edStr2, setEdStr2] = useState(topic.defaultInput.str2 || "");
  ...
  const frames = useMemo(() => {
    try {
      ...
      if (algorithm === "edit-distance") {
        return Array.from(editDistanceGenerator(edStr1, edStr2));
      }
    } catch (e) {
      console.error(e);
      return [];
    ...
    return [];
  }, [algorithm, capacity, items, str1, str2, amount, coins, edStr1, edStr2]);
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl border border-surface-200 bg-white p-5 shadow-sm dark:border-surface-800 dark:bg-surface-900">
            ...
            {algorithm === "edit-distance" && (
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold">String 1</label>
                  <input type="text" maxLength={15} value={edStr1} onChange={(e) => setEdStr1(e.target.value.toUpperCase())} className="block mt-1 p-2 border rounded dark:bg-surface-800 dark:border-surface-700 w-full uppercase" />
                </div>
                <div>
                  <label className="text-sm font-semibold">String 2</label>
                  <input type="text" maxLength={15} value={edStr2} onChange={(e) => setEdStr2(e.target.value.toUpperCase())} className="block mt-1 p-2 border rounded dark:bg-surface-800 dark:border-surface-700 w-full uppercase" />
                </div>
              </div>
            )}
        ...
        {!isEditing && (
          <div className="rounded-2xl border border-surface-200 bg-white p-5 shadow-sm dark:border-surface-800 dark:bg-surface-900 min-h-[400px]">
             ...
             {algorithm === "edit-distance" && renderEditDistanceGrid()}
          </div>
        )}
